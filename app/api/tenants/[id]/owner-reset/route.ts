  ...
  try {
    ...
    if (tenant.slug === PLATFORM_TENANT_SLUG) {
      const response = NextResponse.json(
        { error: "Platform tenant owner reset is not allowed from this action." },
        { status: 409 }
      )
    ...
    if (!owner) {
      const response = NextResponse.json({ error: "Owner user not found." }, { status: 404 })
      logApiRequestSuccess(logContext, 404, { reason: "owner_not_found", tenantId: id })
      return withRequestId(response, logContext.requestId)
    }
    ...
    const response = NextResponse.json({ error: "Unable to send owner reset." }, { status: 500 })
    return withRequestId(response, logContext.requestId)
  }
}
